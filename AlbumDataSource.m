   ...
   /* NSLog(@"Asset Fetched: %@", tmpAlbum.assetsList);
    ...
    for(Album* i in tmp ) {
        ...
        if([i.sortOrder boolValue]==YES){
            album.assetsList=[self simpleQuery:@"Asset" predicate:pre sortField:[i byCondition]  sortOrder:YES];
        }else {
            album.assetsList=[self simpleQuery:@"Asset" predicate:pre sortField:[i byCondition]  sortOrder:NO];
